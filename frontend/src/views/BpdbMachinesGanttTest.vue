<template>
  ...
  <div class="gantt-page" :class="{ 'is-immersive': isBrowserFullscreen }">
    <!-- BẢN TEST TẠM — clone nguyên BpdbMachinesGantt.vue để thử tính năng "đẩy tín hiệu mẻ
         vừa chạy sang /machine-id-board cho nhấp nháy đỏ" (yêu cầu 2026-08-11) mà không đụng
...
// CHỈ có trong bản TEST này (xem ghi chú đầu <template>) — với mỗi mẻ VỪA chuyển "đang
// chạy" mà fetchGantt() vừa phát hiện (newlyAppearedRunningIds), báo cho backend bắn
// broadcast để /machine-id-board nhấp nháy đỏ đúng mã máy. Best-effort: lỗi mạng/API ở đây
// không quan trọng bằng bản thân biểu đồ Gantt vẫn phải hiển thị đúng, nên nuốt lỗi im lặng
// thay vì set errorMsg (không được che mất lỗi tải Gantt thật bằng lỗi của tính năng phụ này).
const notifyNewlyRunning = () => {
  for (const id of newlyAppearedRunningIds) {
    const detail = itemDetails.get(id);
    if (!detail) continue;
    axios.post('/api/public/bpdb-machines-gantt/notify-running', {
      machineCode: detail.machineCode,
      tankLabel: detail.tankLabel || null,
      color: detail.color,
